...
namespace Gregboy {
    public class TIMER {
        ...
        private const int AA_DIV_FREQ = 256;              //16384Hz
        private const int CGB_DIV_FREQ = AA_DIV_FREQ * 2; //32768Hz
        private static readonly int[] TAC_FREQ = { 1024, 16, 64, 256 };
        //00: CPU Clock / 1024 (AA, CGB:   4096 Hz, SGB:   ~4194 Hz)
        //01: CPU Clock / 16   (AA, CGB: 262144 Hz, SGB: ~268400 Hz)
        //10: CPU Clock / 64   (AA, CGB:  65536 Hz, SGB:  ~67110 Hz)
        //11: CPU Clock / 256  (AA, CGB:  16384 Hz, SGB:  ~16780 Hz)
        ...
        private void handleDivider(int cycles, MMU mmu) {
            divCounter += cycles;
            while (divCounter >= AA_DIV_FREQ) {
                mmu.DIV++;
                divCounter -= AA_DIV_FREQ;
            }
        }
