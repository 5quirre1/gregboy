    ...
    class CPU { // Sharp LR35902 CPU
        ...
        private void JR(bool flag) {
            if (flag) {
                ...
                PC = (ushort)(PC + sb);
                PC += 1; //<---- //TODO WHAT?
                cycles += Cycles.JUMPRELATIVETRUE;
            } else {
                PC += 1;
                cycles += Cycles.JUMPRELATIVEFALSE;
            }
        }
        ...
        private void RETURN(bool flag) {
            if (flag) {
                PC = POP();
                cycles += Cycles.RETURNTRUE;
            } else {
                cycles += Cycles.RETURNFALSE;
            }
        }
        ...
        private void CALL(bool flag) {
            if (flag) {
                PUSH((ushort)(PC + 2));
                PC = mmu.readWord(PC);
                cycles += Cycles.CALLTRUE;
            } else {
                PC += 2;
                cycles += Cycles.CALLFALSE;
            }
        }
        ...
        private void JUMP(bool flag) {
            if (flag) {
                PC = mmu.readWord(PC);
                cycles += Cycles.JUMPTRUE;
            } else {
                PC += 2;
                cycles += Cycles.JUMPFALSE;
            }
        }
